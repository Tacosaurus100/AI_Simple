...
void loadImages (std::string inputImageFolder, Matrix3D<float> **inputMatrixes, Matrix3D<float> **outputMatrixes, map<std::string, int>& answerToIndex, std::string type, int size);

void loadImagesHelper (std::string inputImageFolder, Matrix3D<float> **inputMatrixes, Matrix3D<float> **outputMatrixes, map<std::string, int>& answerToIndex, std::string type, int size, int* index);
...
int main (int argc, char *argv[]) {
   ...
   *currentPath += "/../../artificialIntelligence/neuralNetworks";
   ...
   BasicLayerList<float>* model = new artificialIntelligence::classes::BasicLayerList<float> ();
...
void loadImagesHelper (std::string inputImageFolder, Matrix3D<float> **inputMatrixes, Matrix3D<float> **outputMatrixes, map<std::string, int>& answerToIndex, std::string type, int size, int* index) {
   ...
   if (!std::filesystem::is_directory(inputImageFolder)) {
      ...
      inputMatrixes[*index] = images::generate::inputMatrixNormalized(inputImageFolder, type);

      // make the output
      Matrix3D<float>* output = new Matrix3D<float> (1, 1, answerToIndex.size());
